<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="资料标题" prop="ziliaobiaoti">
          <el-input v-model="ruleForm.ziliaobiaoti" 
              placeholder="资料标题" clearable  :readonly="ro.ziliaobiaoti"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="资料标题" prop="ziliaobiaoti">
              <el-input v-model="ruleForm.ziliaobiaoti" 
                placeholder="资料标题" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="资料分类" prop="ziliaofenlei">
          <el-select v-model="ruleForm.ziliaofenlei" placeholder="请选择资料分类">
            <el-option
        ...
        <div v-else>
          <el-form-item class="input" label="资料分类" prop="ziliaofenlei">
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="资料" prop="ziliao">
          <file-upload
          tip="点击上传资料"
          action="file/upload"
          :limit="1"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.ziliao" label="资料" prop="ziliao">
            <el-button type="text" size="small" @click="download(ruleForm.ziliao)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      <el-col :span="12">
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `ziliaoxiazai/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.ziliaoxiazaiCrossAddOrUpdateFlag = false;
                  this.parent.search();
